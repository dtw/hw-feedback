 ...
 */
  ...
  function feedbackstarrating($float,$type = 'individual',$colour = NULL) {
    ...
    // we never want less than one star
		if ($float < 1) {
			echo '<i class="fas fa-star fa-lg' . if ($colour_class) {echo $colour;} . '"></i>';
		} else {
      ...
      if ( $type === 'average' ) {
  			if (($float - floor($float)) >= 0.25 && $float < 0.75) {
  				echo '<i class="fa fa-star-half-empty fa-lg' . if ($colour_class) {echo $colour;} . '"></i>';
  				$star_count++;
  			}
      }
      // add empty stars up to 5
			while ($star_count < 5) {
				echo '<i class="far fa-star fa-lg' . if ($colour_class) {echo $colour;} . '"></i>';
				$star_count++;
			}
		}
  }
